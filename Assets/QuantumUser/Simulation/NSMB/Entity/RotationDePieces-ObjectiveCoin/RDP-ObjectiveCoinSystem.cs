...
namespace Quantum {
    public unsafe class RDPObjectiveCoinSystem : SystemMainThread, ISignalOnMarioPlayerDropObjective, ISignalOnMarioPlayerCollectedCoin,
        ...
        private void HandleSpawningNewStarCoin(Frame f, VersusStageData stage) {
            ...
            for (int i = 0; i < spawnpoints; i++) {
                ...
                if (hits.Count == 0) {
                    // Hit no players
                    var gamemode = (RotationDePiecesGamemode) f.FindAsset(f.Global->Rules.Gamemode);
        ...
        public static void SpawnObjectiveCoins(Frame f, FPVector2 origin, int amount, byte exludeTeam, bool selfDamage) {
            ...
            VersusStageData stage = f.FindAsset<VersusStageData>(f.Map.UserAsset);
            var gamemode = f.FindAsset(f.Global->Rules.Gamemode) as RotationDePiecesGamemode;
            for (int i = 0; i < amount; i++) {
                EntityRef newCoin = f.Create(gamemode.RDPObjectiveCoinPrototype);
                ...
                var coin = f.Unsafe.GetPointer<RDPObjectiveCoin>(newCoin);
        ...
        public void OnMarioPlayerCollectedCoin(Frame f, EntityRef marioEntity, EntityRef coinEntity, FPVector2 worldLocation, QBoolean fromBlock, QBoolean downwards) {
            if (!f.Unsafe.TryGetPointer(coinEntity, out RDPObjectiveCoin* coin)) {
